...
import { useRoqClient, useItemFindFirst } from 'lib/roq';
...
function ItemEditPage() {
  ...
  const { data, error, isLoading, mutate } = useItemFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ItemInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.item.update({
        data: values as RoqTypes.item,
        where: {
          id,
        },
      });
